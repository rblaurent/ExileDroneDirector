...
$requiredFiles = @(
    ...
    'tools\blueprint\Build-StateClipPlanValidationGraph.py',
    'tools\blueprint\Test-StateClipPlanValidationContracts.py',
    'tools\blueprint\snippets\validate-state-clip-plan-v1.eddgraph',
    'tools\blueprint\snippets\validate-state-clip-plan-v1-paste.eddgraph',
    ...
    'tools\unreal\camera_playback_native_application_acceptance_common.py',
    'tools\unreal\Validate-CameraPlaybackNativeApplicationRuntime.py',
    'tools\unreal\Validate-CameraPlaybackNativeApplicationPIE.py',
    ...
    'tools\blueprint\Build-CameraOperatorOverrideLookGraph.py',
    'tools\blueprint\Test-CameraOperatorOverrideLookContracts.py',
    'tools\blueprint\snippets\build-camera-operator-look-v1.eddgraph',
    'tools\blueprint\snippets\build-camera-operator-look-v1-paste.eddgraph',
    'tools\blueprint\Build-CameraOperatorOverrideCommitGraph.py',
...
$stateClipValidation = Join-Path $stateClipRoot 'validate-state-clip-plan-v1.eddgraph'
$stateClipValidationPaste = Join-Path $stateClipRoot 'validate-state-clip-plan-v1-paste.eddgraph'
$stateClipValidationRepeat = Join-Path $stateClipRoot 'validate-state-clip-plan-v1-repeat.eddgraph'
$stateClipValidationRepeatPaste = Join-Path $stateClipRoot 'validate-state-clip-plan-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($stateClipValidation, $stateClipValidationPaste),
    @($stateClipValidationRepeat, $stateClipValidationRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-StateClipPlanValidationGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "State Clip validation generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($stateClipValidation, $stateClipValidationRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\validate-state-clip-plan-v1.eddgraph')),
    @($stateClipValidationPaste, $stateClipValidationRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\validate-state-clip-plan-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "State Clip validation regeneration diverged." }
}
foreach ($graphCase in @(
    @($stateClipValidation, $false),
    @($stateClipValidationPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "State Clip validation link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-StateClipPlanValidationContracts.py') @arguments
    if ($LASTEXITCODE -ne 0) { throw "State Clip validation contracts failed with exit code $LASTEXITCODE." }
}
...
$boundedEventCrossing = Join-Path $boundedEventRoot 'collect-crossed-cues-v1.eddgraph'
$boundedEventCrossingPaste = Join-Path $boundedEventRoot 'collect-crossed-cues-v1-paste.eddgraph'
$boundedEventCrossingRepeat = Join-Path $boundedEventRoot 'collect-crossed-cues-v1-repeat.eddgraph'
$boundedEventCrossingRepeatPaste = Join-Path $boundedEventRoot 'collect-crossed-cues-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($boundedEventCrossing, $boundedEventCrossingPaste),
    @($boundedEventCrossingRepeat, $boundedEventCrossingRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-BoundedEventCrossingCollectionGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Bounded event crossing generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($boundedEventCrossing, $boundedEventCrossingRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\collect-crossed-cues-v1.eddgraph')),
    @($boundedEventCrossingPaste, $boundedEventCrossingRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\collect-crossed-cues-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Bounded event crossing regeneration diverged." }
}
foreach ($graphCase in @(
    @($boundedEventCrossing, $false),
    @($boundedEventCrossingPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Bounded event crossing link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
...
$cameraPlaybackReset = Join-Path $cameraPlaybackRoot 'reset-camera-playback-frame-v1.eddgraph'
$cameraPlaybackResetPaste = Join-Path $cameraPlaybackRoot 'reset-camera-playback-frame-v1-paste.eddgraph'
$cameraPlaybackResetRepeat = Join-Path $cameraPlaybackRoot 'reset-camera-playback-frame-v1-repeat.eddgraph'
$cameraPlaybackResetRepeatPaste = Join-Path $cameraPlaybackRoot 'reset-camera-playback-frame-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackReset, $cameraPlaybackResetPaste),
    @($cameraPlaybackResetRepeat, $cameraPlaybackResetRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackFrameResetGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback-frame reset generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackReset, $cameraPlaybackResetRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\reset-camera-playback-frame-v1.eddgraph')),
    @($cameraPlaybackResetPaste, $cameraPlaybackResetRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\reset-camera-playback-frame-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback-frame reset regeneration diverged." }
}
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackFrameResetContracts.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackReset
if ($LASTEXITCODE -ne 0) { throw "Camera playback-frame reset full contracts failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackReset
if ($LASTEXITCODE -ne 0) { throw "Camera playback-frame reset full links failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackFrameResetContracts.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackResetPaste --paste
if ($LASTEXITCODE -ne 0) { throw "Camera playback-frame reset paste contracts failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
...
$cameraPlaybackSources = Join-Path $cameraPlaybackRoot 'evaluate-camera-playback-sources-v1.eddgraph'
$cameraPlaybackSourcesPaste = Join-Path $cameraPlaybackRoot 'evaluate-camera-playback-sources-v1-paste.eddgraph'
$cameraPlaybackSourcesRepeat = Join-Path $cameraPlaybackRoot 'evaluate-camera-playback-sources-v1-repeat.eddgraph'
$cameraPlaybackSourcesRepeatPaste = Join-Path $cameraPlaybackRoot 'evaluate-camera-playback-sources-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackSources, $cameraPlaybackSourcesPaste),
    @($cameraPlaybackSourcesRepeat, $cameraPlaybackSourcesRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackFrameSourceEvaluatorGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback source-evaluator generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackSources, $cameraPlaybackSourcesRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\evaluate-camera-playback-sources-v1.eddgraph')),
    @($cameraPlaybackSourcesPaste, $cameraPlaybackSourcesRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\evaluate-camera-playback-sources-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback source-evaluator regeneration diverged." }
}
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackFrameSourceEvaluatorContracts.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackSources
if ($LASTEXITCODE -ne 0) { throw "Camera playback source-evaluator full contracts failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackSources
if ($LASTEXITCODE -ne 0) { throw "Camera playback source-evaluator full links failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackFrameSourceEvaluatorContracts.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackSourcesPaste --paste
if ($LASTEXITCODE -ne 0) { throw "Camera playback source-evaluator paste contracts failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
...
$cameraPlaybackCompose = Join-Path $cameraPlaybackRoot 'compose-camera-playback-frame-v1.eddgraph'
$cameraPlaybackComposePaste = Join-Path $cameraPlaybackRoot 'compose-camera-playback-frame-v1-paste.eddgraph'
$cameraPlaybackComposeRepeat = Join-Path $cameraPlaybackRoot 'compose-camera-playback-frame-v1-repeat.eddgraph'
$cameraPlaybackComposeRepeatPaste = Join-Path $cameraPlaybackRoot 'compose-camera-playback-frame-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackCompose, $cameraPlaybackComposePaste),
    @($cameraPlaybackComposeRepeat, $cameraPlaybackComposeRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackFrameComposeGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback compose generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackCompose, $cameraPlaybackComposeRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\compose-camera-playback-frame-v1.eddgraph')),
    @($cameraPlaybackComposePaste, $cameraPlaybackComposeRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\compose-camera-playback-frame-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback compose regeneration diverged." }
}
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackFrameComposeContracts.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackCompose
if ($LASTEXITCODE -ne 0) { throw "Camera playback compose full contracts failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackCompose
if ($LASTEXITCODE -ne 0) { throw "Camera playback compose full links failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackFrameComposeContracts.py') `
    --project-root $ProjectRoot --graph $cameraPlaybackComposePaste --paste
if ($LASTEXITCODE -ne 0) { throw "Camera playback compose paste contracts failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
...
$cameraPlaybackNativeRoot = Join-Path $scratchRoot ("edd-camera-playback-native-" + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $cameraPlaybackNativeRoot -Force | Out-Null
$cameraPlaybackNativeReset = Join-Path $cameraPlaybackNativeRoot 'reset-camera-playback-native-application-v1.eddgraph'
$cameraPlaybackNativeResetPaste = Join-Path $cameraPlaybackNativeRoot 'reset-camera-playback-native-application-v1-paste.eddgraph'
$cameraPlaybackNativeResetRepeat = Join-Path $cameraPlaybackNativeRoot 'reset-camera-playback-native-application-v1-repeat.eddgraph'
$cameraPlaybackNativeResetRepeatPaste = Join-Path $cameraPlaybackNativeRoot 'reset-camera-playback-native-application-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackNativeReset, $cameraPlaybackNativeResetPaste),
    @($cameraPlaybackNativeResetRepeat, $cameraPlaybackNativeResetRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackNativeApplicationResetGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native reset generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackNativeReset, $cameraPlaybackNativeResetRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\reset-camera-playback-native-application-v1.eddgraph')),
    @($cameraPlaybackNativeResetPaste, $cameraPlaybackNativeResetRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\reset-camera-playback-native-application-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback native reset regeneration diverged." }
}
foreach ($graphCase in @(
    @($cameraPlaybackNativeReset, $false),
    @($cameraPlaybackNativeResetPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native reset link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackNativeApplicationResetContracts.py') @arguments
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native reset contracts failed with exit code $LASTEXITCODE." }
}
$cameraPlaybackNativeStage = Join-Path $cameraPlaybackNativeRoot 'stage-camera-playback-native-application-inputs-v1.eddgraph'
$cameraPlaybackNativeStagePaste = Join-Path $cameraPlaybackNativeRoot 'stage-camera-playback-native-application-inputs-v1-paste.eddgraph'
$cameraPlaybackNativeStageRepeat = Join-Path $cameraPlaybackNativeRoot 'stage-camera-playback-native-application-inputs-v1-repeat.eddgraph'
$cameraPlaybackNativeStageRepeatPaste = Join-Path $cameraPlaybackNativeRoot 'stage-camera-playback-native-application-inputs-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackNativeStage, $cameraPlaybackNativeStagePaste),
    @($cameraPlaybackNativeStageRepeat, $cameraPlaybackNativeStageRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackNativeApplicationStageGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native staging generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackNativeStage, $cameraPlaybackNativeStageRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\stage-camera-playback-native-application-inputs-v1.eddgraph')),
    @($cameraPlaybackNativeStagePaste, $cameraPlaybackNativeStageRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\stage-camera-playback-native-application-inputs-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback native staging regeneration diverged." }
}
foreach ($graphCase in @(
    @($cameraPlaybackNativeStage, $false),
    @($cameraPlaybackNativeStagePaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native staging link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackNativeApplicationStageContracts.py') @arguments
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native staging contracts failed with exit code $LASTEXITCODE." }
}
$cameraPlaybackNativeValidation = Join-Path $cameraPlaybackNativeRoot 'validate-camera-playback-native-application-inputs-v1.eddgraph'
$cameraPlaybackNativeValidationPaste = Join-Path $cameraPlaybackNativeRoot 'validate-camera-playback-native-application-inputs-v1-paste.eddgraph'
$cameraPlaybackNativeValidationRepeat = Join-Path $cameraPlaybackNativeRoot 'validate-camera-playback-native-application-inputs-v1-repeat.eddgraph'
$cameraPlaybackNativeValidationRepeatPaste = Join-Path $cameraPlaybackNativeRoot 'validate-camera-playback-native-application-inputs-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackNativeValidation, $cameraPlaybackNativeValidationPaste),
    @($cameraPlaybackNativeValidationRepeat, $cameraPlaybackNativeValidationRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackNativeApplicationValidationGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native preflight generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackNativeValidation, $cameraPlaybackNativeValidationRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\validate-camera-playback-native-application-inputs-v1.eddgraph')),
    @($cameraPlaybackNativeValidationPaste, $cameraPlaybackNativeValidationRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\validate-camera-playback-native-application-inputs-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback native preflight regeneration diverged." }
}
foreach ($graphCase in @(
    @($cameraPlaybackNativeValidation, $false),
    @($cameraPlaybackNativeValidationPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native preflight link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
...
$cameraPlaybackNativeApply = Join-Path $cameraPlaybackNativeRoot 'apply-camera-playback-native-frame-v1.eddgraph'
$cameraPlaybackNativeApplyPaste = Join-Path $cameraPlaybackNativeRoot 'apply-camera-playback-native-frame-v1-paste.eddgraph'
$cameraPlaybackNativeApplyRepeat = Join-Path $cameraPlaybackNativeRoot 'apply-camera-playback-native-frame-v1-repeat.eddgraph'
$cameraPlaybackNativeApplyRepeatPaste = Join-Path $cameraPlaybackNativeRoot 'apply-camera-playback-native-frame-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackNativeApply, $cameraPlaybackNativeApplyPaste),
    @($cameraPlaybackNativeApplyRepeat, $cameraPlaybackNativeApplyRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackNativeApplicationApplyGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native apply generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackNativeApply, $cameraPlaybackNativeApplyRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\apply-camera-playback-native-frame-v1.eddgraph')),
    @($cameraPlaybackNativeApplyPaste, $cameraPlaybackNativeApplyRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\apply-camera-playback-native-frame-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback native apply regeneration diverged." }
}
foreach ($graphCase in @(
    @($cameraPlaybackNativeApply, $false),
    @($cameraPlaybackNativeApplyPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native apply link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackNativeApplicationApplyContracts.py') @arguments
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native apply contracts failed with exit code $LASTEXITCODE." }
}
$cameraPlaybackNativeRestore = Join-Path $cameraPlaybackNativeRoot 'restore-camera-playback-native-state-v1.eddgraph'
$cameraPlaybackNativeRestorePaste = Join-Path $cameraPlaybackNativeRoot 'restore-camera-playback-native-state-v1-paste.eddgraph'
$cameraPlaybackNativeRestoreRepeat = Join-Path $cameraPlaybackNativeRoot 'restore-camera-playback-native-state-v1-repeat.eddgraph'
$cameraPlaybackNativeRestoreRepeatPaste = Join-Path $cameraPlaybackNativeRoot 'restore-camera-playback-native-state-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackNativeRestore, $cameraPlaybackNativeRestorePaste),
    @($cameraPlaybackNativeRestoreRepeat, $cameraPlaybackNativeRestoreRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackNativeApplicationRestoreGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native restore generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackNativeRestore, $cameraPlaybackNativeRestoreRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\restore-camera-playback-native-state-v1.eddgraph')),
    @($cameraPlaybackNativeRestorePaste, $cameraPlaybackNativeRestoreRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\restore-camera-playback-native-state-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback native restore regeneration diverged." }
}
foreach ($graphCase in @(
    @($cameraPlaybackNativeRestore, $false),
    @($cameraPlaybackNativeRestorePaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native restore link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraPlaybackNativeApplicationRestoreContracts.py') @arguments
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native restore contracts failed with exit code $LASTEXITCODE." }
}
$cameraPlaybackNativeCoordinator = Join-Path $cameraPlaybackNativeRoot 'apply-composed-camera-playback-frame-v1.eddgraph'
$cameraPlaybackNativeCoordinatorPaste = Join-Path $cameraPlaybackNativeRoot 'apply-composed-camera-playback-frame-v1-paste.eddgraph'
$cameraPlaybackNativeCoordinatorRepeat = Join-Path $cameraPlaybackNativeRoot 'apply-composed-camera-playback-frame-v1-repeat.eddgraph'
$cameraPlaybackNativeCoordinatorRepeatPaste = Join-Path $cameraPlaybackNativeRoot 'apply-composed-camera-playback-frame-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraPlaybackNativeCoordinator, $cameraPlaybackNativeCoordinatorPaste),
    @($cameraPlaybackNativeCoordinatorRepeat, $cameraPlaybackNativeCoordinatorRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraPlaybackNativeApplicationGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native coordinator generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraPlaybackNativeCoordinator, $cameraPlaybackNativeCoordinatorRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\apply-composed-camera-playback-frame-v1.eddgraph')),
    @($cameraPlaybackNativeCoordinatorPaste, $cameraPlaybackNativeCoordinatorRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\apply-composed-camera-playback-frame-v1-paste.eddgraph'))
)) {
    $hashes = @($comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash })
    if ($hashes[0] -ne $hashes[1] -or $hashes[0] -ne $hashes[2]) { throw "Camera playback native coordinator regeneration diverged." }
}
foreach ($graphCase in @(
    @($cameraPlaybackNativeCoordinator, $false),
    @($cameraPlaybackNativeCoordinatorPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Camera playback native coordinator link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
...
$carrierFrameResetPaste = Join-Path $carrierFrameRoot 'reset-carrier-frame-transport-v1-paste.eddgraph'
$carrierFrameResetRepeat = Join-Path $carrierFrameRoot 'reset-carrier-frame-transport-v1-repeat.eddgraph'
$carrierFrameResetRepeatPaste = Join-Path $carrierFrameRoot 'reset-carrier-frame-transport-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($carrierFrameReset, $carrierFrameResetPaste),
    @($carrierFrameResetRepeat, $carrierFrameResetRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CarrierFrameTransportResetGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame reset generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($carrierFrameReset, $carrierFrameResetRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\reset-carrier-frame-transport-v1.eddgraph')),
    @($carrierFrameResetPaste, $carrierFrameResetRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\reset-carrier-frame-transport-v1-paste.eddgraph'))
)) {
    $hashes = $comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash }
    if (@($hashes | Select-Object -Unique).Count -ne 1) {
        throw 'Carrier-frame reset generation is not byte deterministic.'
    }
}
foreach ($graphCase in @(
    @($carrierFrameReset, $false),
    @($carrierFrameResetPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame reset link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-CarrierFrameTransportResetContracts.py') @arguments
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame reset contracts failed with exit code $LASTEXITCODE." }
}
$carrierFrameStage = Join-Path $carrierFrameRoot 'stage-carrier-frame-transport-inputs-v1.eddgraph'
$carrierFrameStagePaste = Join-Path $carrierFrameRoot 'stage-carrier-frame-transport-inputs-v1-paste.eddgraph'
$carrierFrameStageRepeat = Join-Path $carrierFrameRoot 'stage-carrier-frame-transport-inputs-v1-repeat.eddgraph'
$carrierFrameStageRepeatPaste = Join-Path $carrierFrameRoot 'stage-carrier-frame-transport-inputs-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($carrierFrameStage, $carrierFrameStagePaste),
    @($carrierFrameStageRepeat, $carrierFrameStageRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CarrierFrameTransportStageGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame stage generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($carrierFrameStage, $carrierFrameStageRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\stage-carrier-frame-transport-inputs-v1.eddgraph')),
    @($carrierFrameStagePaste, $carrierFrameStageRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\stage-carrier-frame-transport-inputs-v1-paste.eddgraph'))
)) {
    $hashes = $comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash }
    if (@($hashes | Select-Object -Unique).Count -ne 1) {
        throw 'Carrier-frame stage generation is not byte deterministic.'
    }
}
foreach ($graphCase in @(
    @($carrierFrameStage, $false),
    @($carrierFrameStagePaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame stage link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
...
$carrierFrameTransport = Join-Path $carrierFrameRoot 'build-carrier-frame-transport-samples-v1.eddgraph'
$carrierFrameTransportPaste = Join-Path $carrierFrameRoot 'build-carrier-frame-transport-samples-v1-paste.eddgraph'
$carrierFrameTransportRepeat = Join-Path $carrierFrameRoot 'build-carrier-frame-transport-samples-v1-repeat.eddgraph'
$carrierFrameTransportRepeatPaste = Join-Path $carrierFrameRoot 'build-carrier-frame-transport-samples-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($carrierFrameTransport, $carrierFrameTransportPaste),
    @($carrierFrameTransportRepeat, $carrierFrameTransportRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CarrierFrameTransportSamplesGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame transport generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($carrierFrameTransport, $carrierFrameTransportRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\build-carrier-frame-transport-samples-v1.eddgraph')),
    @($carrierFrameTransportPaste, $carrierFrameTransportRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\build-carrier-frame-transport-samples-v1-paste.eddgraph'))
)) {
    $hashes = $comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash }
    if (@($hashes | Select-Object -Unique).Count -ne 1) {
        throw 'Carrier-frame transport generation is not byte deterministic.'
    }
}
foreach ($graphCase in @(
    @($carrierFrameTransport, $false),
    @($carrierFrameTransportPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame transport link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
...
$carrierFrameCompile = Join-Path $carrierFrameRoot 'compile-carrier-frame-transport-v1.eddgraph'
$carrierFrameCompilePaste = Join-Path $carrierFrameRoot 'compile-carrier-frame-transport-v1-paste.eddgraph'
$carrierFrameCompileRepeat = Join-Path $carrierFrameRoot 'compile-carrier-frame-transport-v1-repeat.eddgraph'
$carrierFrameCompileRepeatPaste = Join-Path $carrierFrameRoot 'compile-carrier-frame-transport-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($carrierFrameCompile, $carrierFrameCompilePaste),
    @($carrierFrameCompileRepeat, $carrierFrameCompileRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CarrierFrameTransportCompileGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame compile generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($carrierFrameCompile, $carrierFrameCompileRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\compile-carrier-frame-transport-v1.eddgraph')),
    @($carrierFrameCompilePaste, $carrierFrameCompileRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\compile-carrier-frame-transport-v1-paste.eddgraph'))
)) {
    $hashes = $comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash }
    if (@($hashes | Select-Object -Unique).Count -ne 1) { throw 'Carrier-frame compile generation is not byte deterministic.' }
}
foreach ($graphCase in @(
    @($carrierFrameCompile, $false),
    @($carrierFrameCompilePaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame compile link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-CarrierFrameTransportCompileContracts.py') @arguments
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame compile contracts failed with exit code $LASTEXITCODE." }
}
$carrierFrameEvaluator = Join-Path $carrierFrameRoot 'evaluate-compiled-carrier-frame-transport-v1.eddgraph'
$carrierFrameEvaluatorPaste = Join-Path $carrierFrameRoot 'evaluate-compiled-carrier-frame-transport-v1-paste.eddgraph'
$carrierFrameEvaluatorRepeat = Join-Path $carrierFrameRoot 'evaluate-compiled-carrier-frame-transport-v1-repeat.eddgraph'
$carrierFrameEvaluatorRepeatPaste = Join-Path $carrierFrameRoot 'evaluate-compiled-carrier-frame-transport-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($carrierFrameEvaluator, $carrierFrameEvaluatorPaste),
    @($carrierFrameEvaluatorRepeat, $carrierFrameEvaluatorRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CarrierFrameTransportEvaluatorGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame evaluator generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($carrierFrameEvaluator, $carrierFrameEvaluatorRepeat, (Join-Path $ProjectRoot 'tools\blueprint\snippets\evaluate-compiled-carrier-frame-transport-v1.eddgraph')),
    @($carrierFrameEvaluatorPaste, $carrierFrameEvaluatorRepeatPaste, (Join-Path $ProjectRoot 'tools\blueprint\snippets\evaluate-compiled-carrier-frame-transport-v1-paste.eddgraph'))
)) {
    $hashes = $comparison | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash }
    if (@($hashes | Select-Object -Unique).Count -ne 1) { throw 'Carrier-frame evaluator generation is not byte deterministic.' }
}
foreach ($graphCase in @(
    @($carrierFrameEvaluator, $false),
    @($carrierFrameEvaluatorPaste, $true)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graphCase[0]
    if ($LASTEXITCODE -ne 0) { throw "Carrier-frame evaluator link integrity failed with exit code $LASTEXITCODE." }
    $arguments = @('--project-root', $ProjectRoot, '--graph', $graphCase[0])
    if ($graphCase[1]) { $arguments += '--paste' }
...
$cameraOperatorLook = Join-Path $cameraOperatorRoot 'build-camera-operator-look-v1.eddgraph'
$cameraOperatorLookPaste = Join-Path $cameraOperatorRoot 'build-camera-operator-look-v1-paste.eddgraph'
$cameraOperatorLookRepeat = Join-Path $cameraOperatorRoot 'build-camera-operator-look-v1-repeat.eddgraph'
$cameraOperatorLookRepeatPaste = Join-Path $cameraOperatorRoot 'build-camera-operator-look-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraOperatorLook,$cameraOperatorLookPaste),
    @($cameraOperatorLookRepeat,$cameraOperatorLookRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraOperatorOverrideLookGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera operator look generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraOperatorLook,$cameraOperatorLookRepeat,(Join-Path $ProjectRoot 'tools\blueprint\snippets\build-camera-operator-look-v1.eddgraph')),
    @($cameraOperatorLookPaste,$cameraOperatorLookRepeatPaste,(Join-Path $ProjectRoot 'tools\blueprint\snippets\build-camera-operator-look-v1-paste.eddgraph'))
)) {
    if ((Get-FileHash -LiteralPath $comparison[0] -Algorithm SHA256).Hash -ne
        (Get-FileHash -LiteralPath $comparison[1] -Algorithm SHA256).Hash -or
        (Get-FileHash -LiteralPath $comparison[0] -Algorithm SHA256).Hash -ne
        (Get-FileHash -LiteralPath $comparison[2] -Algorithm SHA256).Hash) {
        throw 'Camera operator look generation is not byte deterministic.'
    }
}
foreach ($graph in @($cameraOperatorLook,$cameraOperatorLookPaste)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graph
    if ($LASTEXITCODE -ne 0) { throw "Camera operator look link integrity failed with exit code $LASTEXITCODE." }
}
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraOperatorOverrideLookContracts.py') `
    --project-root $ProjectRoot --graph $cameraOperatorLook
if ($LASTEXITCODE -ne 0) { throw "Camera operator look full contracts failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraOperatorOverrideLookContracts.py') `
    --project-root $ProjectRoot --graph $cameraOperatorLookPaste --paste
if ($LASTEXITCODE -ne 0) { throw "Camera operator look paste contracts failed with exit code $LASTEXITCODE." }
$cameraOperatorCommit = Join-Path $cameraOperatorRoot 'commit-camera-operator-override-v1.eddgraph'
$cameraOperatorCommitPaste = Join-Path $cameraOperatorRoot 'commit-camera-operator-override-v1-paste.eddgraph'
$cameraOperatorCommitRepeat = Join-Path $cameraOperatorRoot 'commit-camera-operator-override-v1-repeat.eddgraph'
$cameraOperatorCommitRepeatPaste = Join-Path $cameraOperatorRoot 'commit-camera-operator-override-v1-repeat-paste.eddgraph'
foreach ($pair in @(
    @($cameraOperatorCommit,$cameraOperatorCommitPaste),
    @($cameraOperatorCommitRepeat,$cameraOperatorCommitRepeatPaste)
)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Build-CameraOperatorOverrideCommitGraph.py') `
        --project-root $ProjectRoot --output $pair[0] --paste-output $pair[1]
    if ($LASTEXITCODE -ne 0) { throw "Camera operator commit generation failed with exit code $LASTEXITCODE." }
}
foreach ($comparison in @(
    @($cameraOperatorCommit,$cameraOperatorCommitRepeat,(Join-Path $ProjectRoot 'tools\blueprint\snippets\commit-camera-operator-override-v1.eddgraph')),
    @($cameraOperatorCommitPaste,$cameraOperatorCommitRepeatPaste,(Join-Path $ProjectRoot 'tools\blueprint\snippets\commit-camera-operator-override-v1-paste.eddgraph'))
)) {
    if ((Get-FileHash -LiteralPath $comparison[0] -Algorithm SHA256).Hash -ne
        (Get-FileHash -LiteralPath $comparison[1] -Algorithm SHA256).Hash -or
        (Get-FileHash -LiteralPath $comparison[0] -Algorithm SHA256).Hash -ne
        (Get-FileHash -LiteralPath $comparison[2] -Algorithm SHA256).Hash) {
        throw 'Camera operator commit generation is not byte deterministic.'
    }
}
foreach ($graph in @($cameraOperatorCommit,$cameraOperatorCommitPaste)) {
    & python (Join-Path $ProjectRoot 'tools\blueprint\Test-BlueprintGraphLinkIntegrity.py') `
        --project-root $ProjectRoot --graph $graph
    if ($LASTEXITCODE -ne 0) { throw "Camera operator commit link integrity failed with exit code $LASTEXITCODE." }
}
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraOperatorOverrideCommitContracts.py') `
    --project-root $ProjectRoot --graph $cameraOperatorCommit
if ($LASTEXITCODE -ne 0) { throw "Camera operator commit full contracts failed with exit code $LASTEXITCODE." }
& python (Join-Path $ProjectRoot 'tools\blueprint\Test-CameraOperatorOverrideCommitContracts.py') `
