...
def main():
 ...
 edit=bp.read_blocks(a.project_root/"tools/blueprint/templates/waypoint-edit-node-forms.eddgraph");play=bp.read_blocks(a.project_root/"tools/blueprint/snippets/update-linear-playback.eddgraph");capture=bp.read_blocks(a.project_root/"tools/blueprint/templates/waypoint-capture-node-forms.eddgraph");probe=bp.read_blocks(a.project_root/"tools/blueprint/templates/adaptive-arc-process-node-forms.eddgraph");loop_probe=bp.read_blocks(a.project_root/"tools/blueprint/templates/adaptive-arc-for-loop-with-break-node-form.eddgraph");clean=bp.read_blocks(a.project_root/"tools/blueprint/templates/conan-clean-frame-node-forms.eddgraph");repo=bp.read_blocks(a.project_root/"tools/blueprint/live-snippets/create-private-flypath-v1.eddgraph")
 templ={"length":bp.find_block(edit,r'MemberName="Array_Length"'),"remove":bp.find_block(edit,r'MemberName="Array_Remove"'),"item":bp.find_block(play,r"^Begin Object Class=/Script/BlueprintGraph.K2Node_GetArrayItem"),"add":bp.find_block(capture,r'MemberName="Array_Add"'),"loop":bp.find_block(loop_probe,r"StandardMacros:ForLoopWithBreak"),"vadd":bp.find_block(probe,r'MemberName="Add_VectorVector"'),"vdist":bp.find_block(probe,r'MemberName="Vector_Distance"'),"vmul":bp.find_block(bp.read_blocks(a.project_root/"tools/blueprint/snippets/apply-translation-input.eddgraph"),r'MemberName="Multiply_VectorVector"'),"select":bp.find_block(clean,r"^Begin Object Class=/Script/BlueprintGraph.K2Node_Select "),"call":bp.find_block(repo,r'MemberName="ValidateRecordV1"')}
 def add(key,t,x,y):
 ...
 # Stop as soon as the synchronized work stack is empty or sticky validity has
 # failed.  A fixed ForLoop would burn the entire configured budget as no-ops
 # after completion and trip Unreal's global Blueprint runaway-loop watchdog
 # when this primitive is composed across hundreds of route segments.
 bp.connect(guard,"else",loop,"Break")
 ...
 linear=get("TrajectoryArcBuildInputLinearV1","bool",5120,1216);linear_branch=b.add("linear_branch","branch",5120,1536);bp.connect(stagealpha,"then",linear_branch,"execute");bp.connect(linear,"TrajectoryArcBuildInputLinearV1",linear_branch,"Condition")
 primitive=add("primitive","call",5376,1536);primitive.text=re.sub(r'FunctionReference=\([^\n]*\)','FunctionReference=(MemberName="EvaluateQuinticVectorV1",bSelfContext=True)',primitive.text,1);bp.connect(linear_branch,"else",primitive,"execute");pvalid=get("TrajectoryResultVectorValidV1","bool",5376,1216);pguard=b.add("primitive_guard","branch",5632,1536);bp.connect(primitive,"then",pguard,"execute");bp.connect(pvalid,"TrajectoryResultVectorValidV1",pguard,"Condition")
 p0=get(current_names[2],"vector",4608,720);p1=get(current_names[3],"vector",4608,848);presult=get("TrajectoryResultPositionVectorV1","vector",5632,720)
 setmidp=setv("TrajectoryArcBuildMidpointPositionV1","vector",5888,1536);bp.connect(presult,"TrajectoryResultPositionVectorV1",setmidp,"TrajectoryArcBuildMidpointPositionV1");bp.connect(pguard,"then",setmidp,"execute");fail=setv("TrajectoryArcBuildStageValidV1","bool",5888,1792,"false");bp.connect(pguard,"else",fail,"execute")
 ...
 bp.connect(linear_branch,"then",accept[0],"execute")
