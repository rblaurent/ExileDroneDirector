"""Build the policy-free ordered bounded playback-event dispatcher."""
...
FUNCTION = "DispatchBoundedPlaybackEventsV1"
CALLS = (
...
def main() -> None:
    ...
    spec = importlib.util.spec_from_file_location("edd_event_dispatch_coordinator_base", path)
    ...
    for index, name in enumerate(CALLS):
        node = builder.add(f"call_{name}", "self_call", 256 + index * 288, 0)
        node.text = re.sub(
            r"FunctionReference=\([^\n]*\)",
