"""Build the policy-free ordered camera DOF diagnostic coordinator."""
...
FUNCTION = "EvaluateCameraDofDiagnosticsV1"
...
def main() -> None:
    ...
    spec = importlib.util.spec_from_file_location("edd_camera_dof_evaluate_base", path)
    ...
    for index, name in enumerate(CALLS):
        node = builder.add(f"call_{name}", "self_call", 256 + index * 384, 0)
