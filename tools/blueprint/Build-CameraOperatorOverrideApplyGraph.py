"""Build the policy-free ordered camera-operator override coordinator."""
...
FUNCTION = "ApplyCameraOperatorOverrideV1"
CALLS = (
    "ResetCameraOperatorOverrideStepV1",
    "ValidateCameraOperatorOverrideInputsV1",
    "BuildCameraOperatorTranslationV1",
    "BuildCameraOperatorLookV1",
    "CommitCameraOperatorOverrideV1",
)
...
def main() -> None:
    ...
    spec = importlib.util.spec_from_file_location("edd_camera_operator_apply_base", path)
    scalar = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = scalar
    spec.loader.exec_module(scalar)
    bp = scalar.load_helpers(args.project_root)
    forms = scalar.load_templates(args.project_root, bp)
    source = bp.read_blocks(args.project_root / "tools/blueprint/snippets/activate-drone-view.eddgraph")
    forms["self_call"] = bp.find_block(source, r'MemberName="SwitchToDroneView"')
