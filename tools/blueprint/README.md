...
- `build-adaptive-arc-table-v1.eddgraph` is the checked post-compile
  reset/validate/initialize/process/commit orchestrator. Its deterministic
  source and entryless paste forms are generated by
  `Build-AdaptiveArcCompileGraph.py`; `Test-AdaptiveArcCompileContracts.py`
  rejects any missing, reordered, bypassed, or multiply linked stage.
