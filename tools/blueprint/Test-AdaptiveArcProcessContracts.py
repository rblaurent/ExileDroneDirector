...
def main():
 ...
 expected=119 if not a.paste else 118;c.require(len(nodes)==expected,f"exact {expected}-node process graph")
 ...
 c.require(len(all_("StandardMacros:ForLoop"))==1,"one bounded for-loop")
 loop=one("StandardMacros:ForLoop");match=re.search(r'DefaultValue="([^"]+)"',loop.pins["FirstIndex"].body);c.require(match is None or match.group(1)=="0","loop starts at typed zero")
 ...
 c.require(len(all_('MemberName="Add_VectorVector"'))==1,"linear midpoint vector sum")
 c.require(len(all_('MemberName="Multiply_VectorVector"'))==1,"linear midpoint vector half")
 c.require(len([n for n in nodes.values() if "K2Node_Select" in n.node_class])==1,"linear/quintic midpoint select")
