...
def main():
 ...
 entries=[n for n in nodes.values() if "K2Node_FunctionEntry" in n.node_class];c.require(len(entries)==(0 if x.paste else 1),"entry count");root=nodes["K2Node_VariableSet_0"];c.require(member(root)=="AirframeDocumentAdapterCompileValidV2","commit execution root");c.require(not root.pins["execute"].links,"paste execution root") if x.paste else c.require_link(entries[0],"then",root,"execute","native entry to commit root")
