...
def main():
 a=argparse.ArgumentParser();a.add_argument("--project-root",type=Path,required=True);a.add_argument("--graph",type=Path,required=True);a.add_argument("--paste",action="store_true");x=a.parse_args();c=load(x.project_root/"tools/blueprint/Test-WaypointCaptureContracts.py","edd_camera_channel_stage_graph_base");nodes=c.parse_graph(x.graph);c.require(len(nodes)==(118 if x.paste else 119),f"node count {len(nodes)}");entries=[n for n in nodes.values() if "K2Node_FunctionEntry" in n.node_class];c.require(len(entries)==(0 if x.paste else 1),"entry count");root=nodes["K2Node_VariableSet_0"];c.require(not root.pins["execute"].links,"paste execution root") if x.paste else c.require_link(entries[0],"then",root,"execute","native entry to staging root");text=x.graph.read_text(encoding="utf-8");c.require('MemberName="CompileCameraScalarTrackV1"' not in text,"staging never recompiles");c.require("CameraChannelInputKey" not in text,"staging never reads authored keys");c.require('VariableReference=(MemberName="CameraChannelCompiled' in text and 'K2Node_VariableSet.*CameraChannelCompiled' not in text,"compiled bank read only");c.require(text.count('MemberName="Array_Clear"')==5,"five generic candidate resets");c.require(len([n for n in nodes.values() if "K2Node_MacroInstance" in n.node_class])==1,"one bounded slice loop")
 ...
 string_calls=[n for n in nodes.values() if member(n) in {"EqualEqual_StrStr","NotEqual_StrStr"}];c.require(len(string_calls)==2,"exact string comparison count");c.require(all("KismetStringLibrary" in n.text and "KismetMathLibrary" not in n.text for n in string_calls),"reconstructable string comparisons")
