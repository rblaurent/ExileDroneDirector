...
def main() -> None:
    ...
    invalidators = [node for node in nodes.values() if member(node) == "CameraOperatorResultValidV1" and 'DefaultValue="false"' in node.pins["CameraOperatorResultValidV1"].body]
    publishers = [node for node in nodes.values() if member(node) == "CameraOperatorResultValidV1" and 'DefaultValue="true"' in node.pins["CameraOperatorResultValidV1"].body]
