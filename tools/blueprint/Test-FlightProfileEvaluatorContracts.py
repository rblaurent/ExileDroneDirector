...
def default(n,p):
 body=n.pins[p].body;m=re.search(r'(?:^|,)DefaultValue="([^"]*)"',body);return m.group(1) if m is not None else ("" if 'PinType.PinCategory="string"' in body else None)
def main():
 p=argparse.ArgumentParser();p.add_argument("--project-root",type=Path,required=True);p.add_argument("--graph",type=Path,required=True);p.add_argument("--paste",action="store_true");a=p.parse_args();c=load(a.project_root);ns=c.parse_graph(a.graph);c.require(len(ns)==(181 if a.paste else 182),f"evaluator node count {len(ns)}")
 ...
 rvalid=c.one(ns,'MemberName="FlightProfileResolveResultValidV1"');rid=c.one(ns,'MemberName="FlightProfileResolveResultIdV1"');ideq=c.one(ns,'MemberName="EqualEqual_StrStr"');c.require('/Script/Engine.KismetStringLibrary' in ideq.text,"string equality library");c.require('/Script/Engine.KismetMathLibrary' not in ideq.text,"string equality has no stale MathLibrary self pin");c.require_link(rid,"FlightProfileResolveResultIdV1",ideq,"A","resolved identity");c.require_link(loop,"Array Element",ideq,"B","compiled identity")
 ...
 inclusive_lowers=[n for n in ns.values() if 'MemberName="GreaterEqual_DoubleDouble"' in n.text];strict_lowers=[n for n in ns.values() if 'MemberName="Greater_DoubleDouble"' in n.text];uppers=[n for n in ns.values() if 'MemberName="LessEqual_DoubleDouble"' in n.text];c.require(len(inclusive_lowers)==6 and len(strict_lowers)==4 and len(uppers)==10,"finite range guards")
 boolean_ands=[n for n in ns.values() if 'MemberName="BooleanAND"' in n.text];c.require(len(boolean_ands)==45,"pre/integrity sticky conjunctions")
 scan_items=[];selected_items=[]
 for channel_index,((suf,rs),cg) in enumerate(zip(CHANNELS,compiled)):
  ...
  if suf!="Ids":
   ...
   lower,upper,inclusive=BOUNDS[channel_index-1];lower_nodes=inclusive_lowers if inclusive else strict_lowers;c.require(any(c.linked(scan,"Output",n,"A") and default(n,"B")==lower for n in lower_nodes),f"finite lower {suf}");c.require(any(c.linked(scan,"Output",n,"A") and default(n,"B")==upper for n in uppers),f"finite upper {suf}")
