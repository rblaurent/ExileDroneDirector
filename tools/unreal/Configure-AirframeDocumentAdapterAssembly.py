"""Create and verify the lossless v2 document adapter on Client Director."""
...
PREFIX = "EDD_AIRFRAME_DOCUMENT_ADAPTER_CONFIG"
...
for spec in SCHEMA["variables"]:
    ...
    if name not in created:
        emit("EXISTING_DEFAULT_PRESERVED", f"{name}|COUNT|{len(current)}" if spec["container"] == "Array" else f"{name}|{current}")
        continue
    if spec["container"] == "Array":
        if len(current) != 0:
