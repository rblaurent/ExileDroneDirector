"""Create and verify the typed airframe source-sampling seam on Client Director.

The configurator is intentionally idempotent. Existing fields and function
graphs are preserved, while newly created scalar defaults are written only
after their generated properties are visible on the compiled class.
"""
...
PREFIX = "EDD_AIRFRAME_SOURCE_SAMPLING_CONFIG"
CLIENT = "/Game/Mods/ExileDroneDirector/Core/Client/BPC_EDD_ClientDirector"
ROOT = Path(__file__).resolve().parents[2]
SCHEMA = json.loads(
    (ROOT / "tools/trajectory/airframe_source_sampling_blueprint_schema.json").read_text(
        encoding="utf-8"
    )
)
...
for spec in SCHEMA["variables"]:
    ...
    if spec["container"] == "Array":
        if len(get_property(name)) != 0:
            raise RuntimeError(f"array default not empty: {name}")
        emit("ARRAY_DEFAULT_VERIFIED", name)
        continue
    value = spec["default"]
    if spec["type"] == "Vector":
        set_property(name, unreal.Vector(*(float(component) for component in value)))
    elif spec["type"] == "Quat":
        set_property(name, unreal.Quat(*(float(component) for component in value)))
