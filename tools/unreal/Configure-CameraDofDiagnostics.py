"""Create and verify the camera DOF diagnostic family on Client Director."""
...
for spec in SCHEMA["variables"]:
    ...
    if spec["container"] != "None": raise RuntimeError(f"unexpected container:{name}")
    if not unreal.BlueprintEditorLibrary.add_member_variable(blueprint, name, types[spec["type"]]): raise RuntimeError(f"failed to add {name}")
...
for spec in SCHEMA["variables"]:
    ...
    if name not in created:
        emit("EXISTING_DEFAULT_PRESERVED", f"{name}|{get_property(name)}"); continue
    value = spec["default"]
