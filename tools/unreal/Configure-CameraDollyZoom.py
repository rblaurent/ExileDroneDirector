"""Create and verify the complete camera dolly-zoom helper on Client Director."""
from __future__ import annotations
import json
from pathlib import Path
import unreal

PREFIX="EDD_CAMERA_DOLLY_CONFIG";CLIENT="/Game/Mods/ExileDroneDirector/Core/Client/BPC_EDD_ClientDirector";ROOT=Path(__file__).resolve().parents[2];SCHEMA=json.loads((ROOT/"tools/trajectory/camera_dolly_zoom_blueprint_schema.json").read_text(encoding="utf-8"))
...
if vector_struct is None:raise RuntimeError("native Vector unavailable")
types={"String":unreal.BlueprintEditorLibrary.get_basic_type_by_name("string"),"Float":unreal.BlueprintEditorLibrary.get_basic_type_by_name("real"),"Integer":unreal.BlueprintEditorLibrary.get_basic_type_by_name("int"),"Boolean":unreal.BlueprintEditorLibrary.get_basic_type_by_name("bool"),"Vector":unreal.BlueprintEditorLibrary.get_struct_type(vector_struct)}
created=set()
...
for spec in SCHEMA["variables"]:
    ...
    value=spec["default"]
    if spec["type"]=="String":set_property(name,str(value))
    ...
    elif spec["type"]=="Vector":set_property(name,unreal.Vector(*(float(component) for component in value)))
