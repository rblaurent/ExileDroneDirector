"""Create and verify the complete camera-focus helper on Client Director."""
...
PREFIX = "EDD_CAMERA_FOCUS_CONFIG"
...
types = {
    ...
    "Vector": unreal.BlueprintEditorLibrary.get_struct_type(vector_struct),
}
created = set()
...
for spec in SCHEMA["variables"]:
    ...
    if spec["container"] == "Array":
        if len(get_property(name)) != 0: raise RuntimeError(f"new array default is not empty: {name}")
    ...
    else: raise RuntimeError(f"unsupported type {spec['type']}")
...
for spec in SCHEMA["functions"]:
    name = spec["name"]
    if unreal.BlueprintEditorLibrary.find_graph(blueprint, unreal.Name(name)) is None: raise RuntimeError(name)
