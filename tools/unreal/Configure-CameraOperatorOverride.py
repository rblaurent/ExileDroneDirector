"""Create and verify the viewer-local camera-operator schema on Client Director."""
from __future__ import annotations
import json
from pathlib import Path
import unreal

PREFIX="EDD_CAMERA_OPERATOR_CONFIG";CLIENT="/Game/Mods/ExileDroneDirector/Core/Client/BPC_EDD_ClientDirector";ROOT=Path(__file__).resolve().parents[2];SCHEMA=json.loads((ROOT/"tools/trajectory/camera_operator_override_blueprint_schema.json").read_text(encoding="utf-8"))
...
vector=unreal.load_object(None,"/Script/CoreUObject.Vector");quaternion=unreal.load_object(None,"/Script/CoreUObject.Quat")
if vector is None or quaternion is None:raise RuntimeError("native Vector/Quat unavailable")
types={"Vector":unreal.BlueprintEditorLibrary.get_struct_type(vector),"Quat":unreal.BlueprintEditorLibrary.get_struct_type(quaternion),"String":unreal.BlueprintEditorLibrary.get_basic_type_by_name("string"),"Float":unreal.BlueprintEditorLibrary.get_basic_type_by_name("real"),"Integer":unreal.BlueprintEditorLibrary.get_basic_type_by_name("int"),"Boolean":unreal.BlueprintEditorLibrary.get_basic_type_by_name("bool")}
created=set()
for spec in SCHEMA["variables"]:
    ...
    if not unreal.BlueprintEditorLibrary.add_member_variable(blueprint,name,pin_type):raise RuntimeError("failed to add "+name)
...
for spec in SCHEMA["variables"]:
    ...
    if name not in created:
        current=get_property(name);emit("EXISTING_DEFAULT_PRESERVED",f"{name}|COUNT|{len(current)}" if spec["container"]=="Array" else f"{name}|{current}");continue
    if spec["container"]=="Array":
        if len(get_property(name))!=0:raise RuntimeError("new array default is not empty: "+name)
    ...
    elif spec["type"]=="Float":set_property(name,float(value))
