"""Create and verify the reusable camera scalar-track engine on Client Director."""
...
PREFIX = "EDD_CAMERA_SCALAR_CONFIG"
...
for spec in SCHEMA["variables"]:
    name=spec["name"];current=get_property(name)
    if name not in created:
        emit("EXISTING_DEFAULT_PRESERVED",f"{name}|COUNT|{len(current)}" if spec["container"]=="Array" else f"{name}|{current}");continue
    if spec["container"]=="Array":
        if len(current)!=0:raise RuntimeError(f"new array default is not empty: {name}")
