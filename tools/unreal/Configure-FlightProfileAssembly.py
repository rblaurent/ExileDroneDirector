"""Create and verify the typed flight-profile compilation/evaluation seam."""
...
PREFIX = "EDD_FLIGHT_PROFILE_CONFIG"
CLIENT = "/Game/Mods/ExileDroneDirector/Core/Client/BPC_EDD_ClientDirector"
ROOT = Path(__file__).resolve().parents[2]
SCHEMA = json.loads(
    (ROOT / "tools/trajectory/flight_profile_blueprint_schema.json").read_text(
        encoding="utf-8"
    )
)
...
for spec in SCHEMA["variables"]:
    ...
    if name not in created:
        if spec["container"] == "Array":
            emit("EXISTING_DEFAULT_PRESERVED", f"{name}|COUNT|{len(current)}")
        else:
            emit("EXISTING_DEFAULT_PRESERVED", f"{name}|{current}")
        continue

    if spec["container"] == "Array":
        if len(current) != 0:
            raise RuntimeError(f"new array default is not empty: {name}")
        emit("DEFAULT_VERIFIED", f"{name}|COUNT|0")
        continue

    default = spec["default"]
    if spec["type"] == "String":
        set_value(name, str(default))
    elif spec["type"] == "Boolean":
        set_value(name, bool(default))
    elif spec["type"] == "Integer":
        set_value(name, int(default))
    elif spec["type"] == "Float":
        set_value(name, float(default))
    else:
        raise RuntimeError(f"unsupported scalar type: {spec['type']}")
