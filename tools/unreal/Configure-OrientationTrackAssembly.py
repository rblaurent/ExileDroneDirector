"""Create and verify the typed multi-key orientation assembly seam."""
...
PREFIX = "EDD_ORIENTATION_TRACK_CONFIG"
...
if quat is None or vector is None:
    raise RuntimeError("native Quat/Vector unavailable")
types = {
    "Quat": unreal.BlueprintEditorLibrary.get_struct_type(quat),
...
for spec in SCHEMA["variables"]:
    name = spec["name"]
    if spec["container"] == "Array":
        if len(get_value(name)) != 0:
            raise RuntimeError(f"array default is not empty: {name}")
        emit("ARRAY_DEFAULT_VERIFIED", name)
        continue
    default = spec.get("default")
    if spec["type"] == "Quat":
        set_value(name, unreal.Quat(*default))
