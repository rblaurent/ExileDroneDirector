"""Converge camera dolly-zoom class defaults after live probes."""
from __future__ import annotations
import json
from pathlib import Path
import unreal

PREFIX="EDD_CAMERA_DOLLY_DEFAULTS";CLIENT="/Game/Mods/ExileDroneDirector/Core/Client/BPC_EDD_ClientDirector";ROOT=Path(__file__).resolve().parents[2];SCHEMA=json.loads((ROOT/"tools/trajectory/camera_dolly_zoom_blueprint_schema.json").read_text(encoding="utf-8"))
...
def expected(spec):
    ...
    value=spec["default"]
    if spec["type"]=="Vector":return unreal.Vector(*(float(component) for component in value))
...
# Compilation reinstantiates the generated class; never write defaults before it.
