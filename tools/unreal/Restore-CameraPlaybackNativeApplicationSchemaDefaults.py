"""Converge playback-native application class defaults after live probes."""
from __future__ import annotations
import json
from pathlib import Path
import unreal

PREFIX="EDD_CAMERA_PLAYBACK_NATIVE_DEFAULTS";CLIENT="/Game/Mods/ExileDroneDirector/Core/Client/BPC_EDD_ClientDirector";ROOT=Path(__file__).resolve().parents[2];SCHEMA=json.loads((ROOT/"tools/trajectory/camera_playback_native_application_blueprint_schema.json").read_text(encoding="utf-8"))
...
def expected(spec):
 value=spec["default"]
 if spec["type"]=="Vector":return unreal.Vector(*(float(v) for v in value))
 if spec["type"]=="Quat":return unreal.Quat(*(float(v) for v in value))
 if spec["type"]=="Transform":return unreal.Transform()
...
def field(value,*names):
 ...
 raise RuntimeError("missing Transform field:"+str(names))
def normalized(value):
 ...
 if isinstance(value,unreal.Transform):return normalized(field(value,"translation","location")),normalized(field(value,"rotation")),normalized(field(value,"scale3d","scale"))
 return value
