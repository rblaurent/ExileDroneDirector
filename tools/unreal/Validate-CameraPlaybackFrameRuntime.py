...
try:
 ...
 for index,(elapsed,x,complete) in enumerate(cases):
  common.set_(obj,"CameraPlaybackInputElapsedSecondsV1",elapsed);obj.call_method("ComposeCameraPlaybackFrameV1");require(bool(common.get(obj,"CameraPlaybackResultValidV1")),f"case:{index}:valid");require(close(common.norm(common.get(obj,"CameraPlaybackResultPositionV1"))[0],x),f"case:{index}:position");require(bool(common.get(obj,"CameraPlaybackResultCompleteV1"))==complete,f"case:{index}:complete");require(common.norm(common.get(obj,"CameraPlaybackResultBodyWorldQuatV1"))==common.norm(common.BODY),f"case:{index}:body");require(common.norm(common.get(obj,"CameraPlaybackResultGimbalWorldQuatV1"))==common.norm(common.GIMBAL),f"case:{index}:gimbal");body=common.norm(common.get(obj,"CameraPlaybackResultBodyWorldQuatV1"));relative=common.norm(common.get(obj,"CameraPlaybackResultGimbalRelativeQuatV1"));gimbal=common.norm(common.get(obj,"CameraPlaybackResultGimbalWorldQuatV1"));require(all(close(a,b) for a,b in zip(quat_mul(body,relative),gimbal)),f"case:{index}:reconstruct");require(len(common.get(obj,"CameraPlaybackResultChannelValuesV1"))==13 and len(common.get(obj,"CameraPlaybackResultComfortEffectiveWeightsV1"))==5,f"case:{index}:shape");require(str(common.get(obj,"CameraPlaybackResultFilmbackPresetIdV1"))=="playback_full_frame",f"case:{index}:filmback");require(common.snapshot(obj,COMPILED)==compiled,f"case:{index}:compiled mutated")
