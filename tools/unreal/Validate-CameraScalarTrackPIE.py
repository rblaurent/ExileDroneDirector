...
PREFIX="EDD_CAMERA_SCALAR_PIE";SOURCE_LEVEL_PATH="/Game/Dev/AlmostEmpty";WORLD_PATH="/Game/Dev/UEDPIE_0_AlmostEmpty.AlmostEmpty";CLIENT_CLASS_PATH="/Game/Mods/ExileDroneDirector/Core/Client/BPC_EDD_ClientDirector.BPC_EDD_ClientDirector_C";TIMEOUT_SECONDS=120.0;ROOT=Path(__file__).resolve().parents[2];SCHEMA=json.loads((ROOT/"tools/trajectory/camera_scalar_track_blueprint_schema.json").read_text(encoding="utf-8"));STATE_NAMES=tuple(spec["name"] for spec in SCHEMA["variables"]);ARRAY_NAMES=frozenset(spec["name"] for spec in SCHEMA["variables"] if spec["container"]=="Array")
...
def run_checks():
    component=director(pie_world());before=tuple(clone(name,get(component,name)) for name in INPUTS);component.call_method("CompileCameraScalarTrackV1");require(bool(get(component,"CameraScalarTrackCompileValidV1")),"compile");component.call_method("EvaluateCameraScalarTrackV1");require(bool(get(component,"CameraScalarTrackResultValidV1")),"evaluate");require(close(get(component,"CameraScalarTrackResultValueV1"),160.0),"optical midpoint");require(int(get(component,"CameraScalarTrackResultSegmentIndexV1"))==0,"segment");require(close(get(component,"CameraScalarTrackResultLocalAlphaV1"),.5),"alpha");require(tuple(clone(name,get(component,name)) for name in INPUTS)==before,"inputs mutated");emit("OPTICAL_MIDPOINT",get(component,"CameraScalarTrackResultValueV1"));emit("GAME_WORLD_RESULT","PASS")
def restore(state):
    if state.get("restored") or not state.get("originals"):return
    for name,value in state["originals"].items():set_(state["defaults"],name,clone(name,value))
...
def tick(_delta):
    ...
    try:
        ...
        if state["stage"]=="prepare":
            require(not subsystem.is_in_play_in_editor(),"PIE already running");require(subsystem.load_level(SOURCE_LEVEL_PATH),"load level");state["defaults"]=defaults();state["originals"]={name:clone(name,get(state["defaults"],name)) for name in STATE_NAMES};stage(state["defaults"]);state["stage"]="request";state["stage_at"]=time.monotonic();emit("SOURCE_LEVEL_READY",SOURCE_LEVEL_PATH);return
